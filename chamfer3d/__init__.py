...
class ChamferFunction(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, xyz1, xyz2, use_mixed_precision=False):
        """前向传播计算"""
        # 检查设备
        ...
        # 检查是否支持混合精度
        if use_mixed_precision and hasattr(_C, "is_mixed_precision_supported"):
            use_mixed_precision = _C.is_mixed_precision_supported()
        ...
        # 创建输出tensor
        dtype = torch.half if use_mixed_precision else torch.float
        ...
        # 根据是否使用混合精度选择对应的前向计算函数
        if use_mixed_precision:
            # 检查函数是否存在
            if not hasattr(_C, "chamfer_forward_mixed_precision"):
                raise RuntimeError("混合精度前向函数未找到，请确认是否正确编译")
    ...
    def backward(ctx, grad_dist1, grad_dist2, grad_idx1, grad_idx2):
        ...
        # 根据是否使用混合精度选择对应的反向计算函数
        if use_mixed_precision:
            # 检查函数是否存在
            if not hasattr(_C, "chamfer_backward_mixed_precision"):
                raise RuntimeError("混合精度反向函数未找到，请确认是否正确编译")
...
class ChamferDistance(torch.nn.Module):
    ...
    def __init__(self, use_mixed_precision=False):
        ...
        Args:
            use_mixed_precision: 是否使用混合精度计算 (仅在支持的GPU上可用)
        """
        super(ChamferDistance, self).__init__()
        # 检查是否支持混合精度
        if use_mixed_precision and hasattr(_C, "is_mixed_precision_supported"):
            self.use_mixed_precision = _C.is_mixed_precision_supported()
        else:
    ...
    def forward(self, xyz1: Tensor, xyz2: Tensor) -> tuple:
        ...
        # 转换数据类型并使用自动求导函数计算
        return ChamferFunction.apply(xyz1, xyz2, self.use_mixed_precision)

# 支持torch.compile
def forward(xyz1: Tensor, xyz2: Tensor, use_mixed_precision: bool = False) -> tuple:
    """函数式API，计算两个点云之间的Chamfer距离
    
    Args:
        xyz1: 第一个点云，形状为(B, N, 3)
        xyz2: 第二个点云，形状为(B, M, 3)
        use_mixed_precision: 是否使用混合精度计算
    
    Returns:
        tuple: (dist1, dist2, idx1, idx2)
    """
    module = ChamferDistance(use_mixed_precision=use_mixed_precision)
